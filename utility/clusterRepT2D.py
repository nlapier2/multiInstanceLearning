...
def option1(indir, output, alpha, alphaopt, verbose):
    ...
    if alphaopt == 2:
        print "alphaopt 2 is not used with option 1"
        return
    ...
    for filename in os.listdir(indir):
        if filename.endswith('.fasta'):
            ...
            fasta = open(indir + filename)
            count = 0   # used for selecting 1 % alpha lines
            for line in fasta:
                    count = 1 + (count % alpha)
                    if count == 1:  # this occurs every (1 / alpha) lines
                        outfile.write(line)   # write the defline
                        read = fasta.next()    # go to read line
                        outfile.write(read)   # write the read
                    else:
...
def create_representation(indir, output, alpha, alphaopt, option, uclustid, uclustpath, verbose):
    ...
    if option == 1:
        option1(indir, output, alpha, alphaopt, verbose)
    elif option == 2:
...
def main():
    ...
    if not indir.endswith('/'):
        indir += '/'
    uclustpath = args.uclustpath
    if not uclustpath.startswith('/'):
        uclustpath = indir + uclustpath
    if args.verbose:
        print "Verbose output requested."
    create_representation(indir, args.output, alpha, alphaopt, option,  uclustid, uclustpath, args.verbose)
    if args.verbose:
