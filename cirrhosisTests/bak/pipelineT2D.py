...
def execute_misvm(verbose, directory, output, positive, negative, cluster, infile, mapfile, split):
    ...
    for line in f:
        if line.startswith('H') or line.startswith('S'):
            ...
            if vec is not None:
                veclen = len(vec)               # current length of vector's feature list
                featnum = int(fields[1])        # number of feature is 1 + cluster number
                if veclen < featnum + 1:        # used to expand feature list if needed
                    for j in range(veclen, featnum + 1):
                        vec.append('')
            '''
    ...
    '''for i in range(len(bags)-1):
        # print bags[i].shape
        if bags[i].dtype != bags[i+1].dtype or bags[i].shape != bags[i+1].shape:
            print '--->ALERT!!!!!'
            print bags[i].dtype
            print bags[i+1].dtype
            print bags[i].shape
            print bags[i+1].shape
            print bags[i]
            print bags[i+1]
            print i'''
